...
namespace DarkTreeFPS
{
    ...
    public class WeaponDataCustomInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            else if(settingSO.weaponType == WeaponType.RocketLauncher)
            {
                DrawRocketLauncher();
            }
        ...
        public void DrawGeneral()
        {
            ...
            if (settingSO.weaponType == WeaponType.Shotgun)
            {
                GUILayout.Label("Shotgun cartridge loading clip");
                settingSO.shotgunLoadingClip = (AudioClip)EditorGUILayout.ObjectField(settingSO.shotgunLoadingClip, typeof(AudioClip), false);
            }
            GUILayout.Label("Recoil Vector3");
            settingSO.recoil = EditorGUILayout.Vector2Field("Recoil", settingSO.recoil);
        ...
        public void DrawFirearms()
        {
            ...
            GUILayout.Label("Will use UI scope");
            settingSO.canUseScope = EditorGUILayout.Toggle(settingSO.canUseScope);
            GUILayout.Label("Time to activate scope");
            settingSO.timeToActivateScope = EditorGUILayout.FloatField(settingSO.timeToActivateScope);

            GUILayout.Label("Weapon shot NPC detection distance");
            settingSO.weaponShotNPCDetectionDistance = EditorGUILayout.FloatField(settingSO.weaponShotNPCDetectionDistance);
            ...
            GUILayout.Label("Shell ejecting force");
        ...
        public void DrawRocketLauncher()
        {
            GUILayout.Label("Rocket launcher settings", EditorStyles.boldLabel);
            GUILayout.BeginVertical("HelpBox");
            /// Main
            GUILayout.Label("Main settings", EditorStyles.centeredGreyMiniLabel);
            GUILayout.Label("Rockets pool size");
            settingSO.projectilePoolSize = EditorGUILayout.IntSlider(settingSO.projectilePoolSize, 1, 100);
