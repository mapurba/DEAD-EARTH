    ...
    public class PickupItem : MonoBehaviour
    {
        ...
        public void PickupWithRaycast()
        {
            ...
            if (Physics.Raycast(playerCamera.transform.position, playerCamera.transform.forward, out hit, raycastPickupDistance))
            {
                ...
                if (hit.collider.CompareTag("Flesh") && hit.collider.GetComponent<PlayerHitTarget>() != null && !InventoryManager.showInventory)
                {
                    //if (itemNameTooltip)
                    //    itemNameTooltip.text = "Search";

                    if (Input.GetKeyDown(pickupKey))
                        inventory.SearchLootBox(hit.collider.GetComponent<PlayerHitTarget>().lootBox);
                }
